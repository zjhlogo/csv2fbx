...
Quaternion QuaternionInterpolate(const Quaternion& q0, const Quaternion& q1, float t)
{
    ...
    res.x = static_cast<float>(scale1 * q0.x + scale2 * q1.x);
    res.y = static_cast<float>(scale1 * q0.y + scale2 * q1.y);
    res.z = static_cast<float>(scale1 * q0.z + scale2 * q1.z);
    res.w = static_cast<float>(scale1 * q0.w + scale2 * q1.w);
    return res;
}
...
Quaternion QuaternionExtrapolate(const Quaternion& q0, const Quaternion& q1, float t)
{
    ...
    res.x = static_cast<float>(s1 * q1.x - s2 * q0.x);
    res.y = static_cast<float>(s1 * q1.y - s2 * q0.y);
    res.z = static_cast<float>(s1 * q1.z - s2 * q0.z);
    res.w = static_cast<float>(s1 * q1.w - s2 * q0.w);
...
Quaternion* QuaternionRotationMatrix(Quaternion* pOut, const Matrix33* pM)
{
    ...
    if (trace > static_cast<float>(0.0))
    {
        float s = sqrt(trace + static_cast<float>(1.0));
        temp[3] = (s * static_cast<float>(0.5));
        s = static_cast<float>(0.5) / s;
    ...
    {
        int i = pM->x.x < pM->y.y ? (pM->y.y < pM->z.z ? 2 : 1) : (pM->x.x < pM->z.z ? 2 : 0);
        int j = (i + 1) % 3;
        int k = (i + 2) % 3;

        float s = sqrt((*pM)(i, i) - (*pM)(j, j) - (*pM)(k, k) + static_cast<float>(1.0));
        temp[i] = s * static_cast<float>(0.5);
        s = static_cast<float>(0.5) / s;
...
void MatrixDecompose(Vector3* pOut, Quaternion* pOutRotation, Vector3* pOutTranslation, const Matrix44* pM)
{
    ...
    *pOutTranslation = pM->w.xyz();
    auto tmpMat33 =
        Matrix33(pM->x.xyz() * (1.0f / pOut->x), pM->y.xyz() * (1.0f / pOut->y), pM->z.xyz() * (1.0f / pOut->z));
...
Plane* PlaneTransform(Plane* pOut, const Plane* pP, const Matrix44* pM)
{
    pOut->normal.x =
        (*pM)(0, 0) * pP->normal.x + (*pM)(1, 0) * pP->normal.y + (*pM)(2, 0) * pP->normal.z + (*pM)(3, 0) * pP->dist;
    pOut->normal.y =
        (*pM)(0, 1) * pP->normal.x + (*pM)(1, 1) * pP->normal.y + (*pM)(2, 1) * pP->normal.z + (*pM)(3, 1) * pP->dist;
    pOut->normal.z =
        (*pM)(0, 2) * pP->normal.x + (*pM)(1, 2) * pP->normal.y + (*pM)(2, 2) * pP->normal.z + (*pM)(3, 2) * pP->dist;
    pOut->dist =
        (*pM)(0, 3) * pP->normal.x + (*pM)(1, 3) * pP->normal.y + (*pM)(2, 3) * pP->normal.z + (*pM)(3, 3) * pP->dist;
    return pOut;
...
Quaternion* QuaternionDecompress(Quaternion* out, short s0, short s1, short s2)
{
    ...
    static constexpr float scale = 1.0f / 32767.0f / 1.41421f;
...
{
    (*pOut)(0, 0) = -2 * pPlane->normal.x * pPlane->normal.x + 1;
    (*pOut)(0, 1) = -2 * pPlane->normal.y * pPlane->normal.x;
    (*pOut)(0, 2) = -2 * pPlane->normal.z * pPlane->normal.x;
    (*pOut)(0, 3) = 0;
    (*pOut)(1, 0) = -2 * pPlane->normal.x * pPlane->normal.y;
    (*pOut)(1, 1) = -2 * pPlane->normal.y * pPlane->normal.y + 1;
    (*pOut)(1, 2) = -2 * pPlane->normal.z * pPlane->normal.y;
    (*pOut)(1, 3) = 0;
    (*pOut)(2, 0) = -2 * pPlane->normal.x * pPlane->normal.z;
    (*pOut)(2, 1) = -2 * pPlane->normal.y * pPlane->normal.z;
    (*pOut)(2, 2) = -2 * pPlane->normal.z * pPlane->normal.z + 1;
    (*pOut)(2, 3) = 0;
    (*pOut)(3, 0) = -2 * pPlane->normal.x * pPlane->dist;
    (*pOut)(3, 1) = -2 * pPlane->normal.y * pPlane->dist;
    (*pOut)(3, 2) = -2 * pPlane->normal.z * pPlane->dist;
    (*pOut)(3, 3) = 1;
    return pOut;
}
...
Matrix44* MatrixClipProj(Matrix44* pOutProj, Matrix44* pViewMat, Matrix44* pOldProjMat, const Plane* pOldClipPlane)
{
    ...
    if (projClipPlane.dist > 0)
    {
        ...
        Plane clipPlane(-pOldClipPlane->normal.x,
                        -pOldClipPlane->normal.y,
                        -pOldClipPlane->normal.z,
                        -pOldClipPlane->dist);
        // transform clip plane into projection space
...
bool LineIntersectLine(Vector2 lineStart0,
                       ...
                       Vector2& outPt)
{
    float denom = ((lineEnd1.y - lineStart1.y) * (lineEnd0.x - lineStart0.x))
                  - ((lineEnd1.x - lineStart1.x) * (lineEnd0.y - lineStart0.y));
    float numerator = ((lineEnd1.x - lineStart1.x) * (lineStart0.y - lineStart1.y))
                      - ((lineEnd1.y - lineStart1.y) * (lineStart0.x - lineStart1.x));

    float numerator2 = ((lineEnd0.x - lineStart0.x) * (lineStart0.y - lineStart1.y))
                       - ((lineEnd0.y - lineStart0.y) * (lineStart0.x - lineStart1.x));
