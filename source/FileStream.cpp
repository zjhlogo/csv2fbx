...
{
    return (0 != (static_cast<unsigned long>(cap) & m_eFileAccesssMode));
}
...
CFileStream::CFileStream()
    : m_currentStatus(EFILEOP_CLOSED)
    , m_eFileAccesssMode(EFILEACCESS_READ)
{
...
//-----------------------------------------------------------------------------
CFileStream::EFILEOP_STATUS CFileStream::SetStatus(EFILEOP_STATUS status)
{
...
{
    char tmp = 0;
    if (Read(sizeof(char), &tmp) == EFILEOP_OK)
    {
        value = tmp;
        return true;
    }
...
{
    char bValue = value;
    if (Write(sizeof(char), &bValue) == EFILEOP_OK)
    {
        return true;
    }
    return false;
...
{
    int len = string ? static_cast<unsigned long>(strlen(string)) : 0;
    if (len > maxLen)
    {
        return false;
    }

    if (!WriteWORD(static_cast<unsigned short>(len)))
    {
        return false;
    }
    if (len)
...
bool CFileStream::ReadString(char* buf, int bufSize)
{
    ...
    return Read(len, buf) == EFILEOP_OK;
}
...
bool CFileStream::ReadLine(std::string& outStr)
{
    ...
    int linesize = 0;
    while (m_pMemBuffer[m_iMemPosition + linesize] != '\n' && m_pMemBuffer[m_iMemPosition + linesize] != '\r'
           && m_iMemPosition + linesize < m_dwMemSize)
    {
        linesize++;
    }
...
CFileStream::EFILEOP_STATUS CSynFileStream::Open(const char* filename, const EFILEACCESS_MODE openMode, bool bStream)
{
    ...
    case EFILEACCESS_READWRITE:
        fopen_s(&m_handle, m_strFileName.c_str(), "a+b"); //����ļ������ڣ��Զ�����
        if (m_handle)
        {
            fclose(m_handle);
        }
    ...
    if (m_handle == nullptr) // handle not created successfully
    {
        return ResetStatus();
    }
    ...
    if (!bStream && m_eFileAccesssMode & static_cast<unsigned long>(EFILEACCESS_MODE_FLAG_READ))
    {
        delete[] m_pMemBuffer;
...
CFileStream::EFILEOP_STATUS CSynFileStream::SetPosition(int position, bool absolutePos)
{
    ...
    if (m_bStream)
    {
        ...
        {
            // relative position
            assert((GetPosition() >= static_cast<unsigned long>(abs(position)) && 0 > position) || 0 <= position);
...
CFileStream::EFILEOP_STATUS CSynFileStream::Read(unsigned long size, char* dst, unsigned long* bytesRead)
{
    if (m_bStream)
    {
        ...
        unsigned long* bytes = (nullptr == bytesRead) ? &lastBytes : bytesRead;
        *bytes = fread(dst, sizeof(unsigned char), size, m_handle);
        if (size != *bytes) //����Ӧ����ѭ�����ã���û����ȫ���ģ��Ժ󿴵��ٸ�
        {
            if (feof(m_handle) != 0)
            {
...
CFileStream::EFILEOP_STATUS CSynFileStream::Write(unsigned long size, const char* src, unsigned long* bytesWritten)
{
    ...
    unsigned long lastBytes;
    unsigned long* bytes = (nullptr == bytesWritten) ? &lastBytes : bytesWritten;
...
{
    CFileStream* pFileStream = nullptr;
    switch (type)
    {
