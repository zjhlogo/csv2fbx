...
void ConvertCSV2FBX(const char* sourceCSVFile)
{
    ...
    m_ioSettings->SetBoolProp(EXP_ASCIIFBX, true);

    // m_ioSettings->SetIntProp(EXP_FBX_EXPORT_FILE_VERSION, FBX_FILE_VERSION_7400);

    sdkManager->SetIOSettings(m_ioSettings);
    scene = FbxScene::Create(sdkManager, "");

    auto pSrcFile = new CCSVFile(sourceCSVFile);
    ...
    for (int iRow = 0; iRow < pSrcFile->GetRowNum(); iRow++)
    {
        ...
        {
            pSrcFile->GetCellValue("POSITION.x", iRow, mVerticsSet[iVertexID].x);
            pSrcFile->GetCellValue("POSITION.y", iRow, mVerticsSet[iVertexID].y);
            pSrcFile->GetCellValue("POSITION.z", iRow, mVerticsSet[iVertexID].z);

            pSrcFile->GetCellValue("NORMAL.x", iRow, mVerticsSet[iVertexID].nx);
            pSrcFile->GetCellValue("NORMAL.y", iRow, mVerticsSet[iVertexID].ny);
            pSrcFile->GetCellValue("NORMAL.z", iRow, mVerticsSet[iVertexID].nz);

            pSrcFile->GetCellValue("TANGENT.x", iRow, mVerticsSet[iVertexID].tx);
            pSrcFile->GetCellValue("TANGENT.y", iRow, mVerticsSet[iVertexID].ty);
            pSrcFile->GetCellValue("TANGENT.z", iRow, mVerticsSet[iVertexID].tz);
            pSrcFile->GetCellValue("TANGENT.w", iRow, mVerticsSet[iVertexID].tw);

            pSrcFile->GetCellValue("TEXCOORD0.x", iRow, mVerticsSet[iVertexID].u);
            pSrcFile->GetCellValue("TEXCOORD0.y", iRow, mVerticsSet[iVertexID].v);
        }
    }
    ...
    auto pluginReg = sdkManager->GetIOPluginRegistry();
    if (pluginReg)
    {
        for (int i = 0; i < pluginReg->GetWriterFormatCount(); ++i)
        {
            std::cout << pluginReg->GetWriterFormatDescription(i) << std::endl;
        }
    }

    int format = sdkManager->GetIOPluginRegistry()->FindWriterIDByDescription("FBX 6.0 ascii (*.fbx)");

    if (!exporter->Initialize(fbxFilePath.c_str(), format, sdkManager->GetIOSettings()))
    {
        std::cerr << "Failed to initialize FBX exporter\n";
        exporter->Destroy();
        return;
    }
    ...
    {
        std::cerr << "Failed to produce FBX file\n";
        exporter->Destroy();
        return;
    }
