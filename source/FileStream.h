...
public:
    EFILEACCESS_MODE GetAccessMode() { return m_eFileAccesssMode; }

    EFILESTREAM_TYPE GetType() { return m_Type; }

    unsigned char* GetContent() { return m_pMemBuffer; }

    //! name	Open.
    //! param	const char * filename -
    //! param	const EFILEACCESS_MODE openMode -
    //! param	bool bStream -bStream if this param set true ,the file is open as a stream if this param set false,the
    //! file will be read to the memory note		Opens a file for access using the specified EFILEACCESS_MODE
    //! return 	Status The status of the file
    virtual EFILEOP_STATUS Open(const char* filename, EFILEACCESS_MODE openMode, bool bStream = true) = 0;
    ...
    virtual unsigned long GetSize() const { return m_dwMemSize; }
    ...
    /// @returns The status of the file
    virtual EFILEOP_STATUS Read(unsigned long size, char* dst, unsigned long* bytesRead = nullptr) = 0;

    /// Writes "size" bytes into the file from the pointer "src".
    /// The number of actual bytes written is returned in bytesWritten
    /// @returns The status of the file
    virtual EFILEOP_STATUS Write(unsigned long size, const char* src, unsigned long* bytesWritten = nullptr) = 0;
...
public:
    ...
    ~CSynFileStream() override; ///< Destructor

    /// Opens a file for access using the specified EFILEACCESS_MODE
    ///
    /// @returns The status of the file
    EFILEOP_STATUS Open(const char* filename, EFILEACCESS_MODE openMode, bool bStream = true) override;
    ...
    EFILEOP_STATUS SetPosition(int position, bool absolutePos = true) override;
    ...
    ///
    /// @returns The status of the file.
    EFILEOP_STATUS Flush() override;

    /// Closes the file
    ///
    /// @returns The status of the file.
    EFILEOP_STATUS Close() override;

    /// Gets the status of the file
    EFILEOP_STATUS GetStatus() const override;

    /// Reads "size" bytes from the file, and dumps data into "dst".
    /// The number of actual bytes read is returned in bytesRead
    /// @returns The status of the file
    EFILEOP_STATUS Read(unsigned long size, char* dst, unsigned long* bytesRead = nullptr) override;

    /// Writes "size" bytes into the file from the pointer "src".
    /// The number of actual bytes written is returned in bytesWritten
    /// @returns The status of the file
    EFILEOP_STATUS Write(unsigned long size, const char* src, unsigned long* bytesWritten = nullptr) override;

protected:
    EFILEOP_STATUS ResetStatus() override; ///< Called after error encountered.
    void NotifyMasterLoaded() override{};
