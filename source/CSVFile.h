...
public:
    ...
    ~CCSVFileRow() { m_vecData.clear(); }
    const std::string& operator[](std::size_t index) const { return m_vecData[index]; }
    std::string& operator[](std::size_t index) { return m_vecData[index]; }

    std::size_t Size() const { return m_vecData.size(); }
...
public:
    ...
    std::map<std::string, int>& GetFieldsMap() { return m_mapFields; }

    std::vector<CCSVFileRow>& GetRowsVector() { return m_vecRows; }
